...
export async function GET() {
  ...
  try {
    ...
    const statements = [
      ...
      `CREATE TABLE IF NOT EXISTS "Episode" ("id" TEXT NOT NULL PRIMARY KEY, "animeId" TEXT NOT NULL, "number" INTEGER NOT NULL, "title" TEXT, "aired" TEXT, "filler" BOOLEAN NOT NULL DEFAULT false, "recap" BOOLEAN NOT NULL DEFAULT false, "hasSub" BOOLEAN NOT NULL DEFAULT false, "hasDub" BOOLEAN NOT NULL DEFAULT false, "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT "Episode_animeId_number_key" UNIQUE ("animeId", "number"), FOREIGN KEY ("animeId") REFERENCES "Anime"("id") ON DELETE CASCADE)`,
    ...
    // Migrations: add hasSub/hasDub to Episode if missing (idempotent — ignores "duplicate column" error).
    for (const col of ['"hasSub"', '"hasDub"']) {
      try { await libsql.execute(`ALTER TABLE "Episode" ADD COLUMN ${col} BOOLEAN NOT NULL DEFAULT false`); results.push(`+Episode.${col}`); } catch (e) { /* column already exists */ }
    }
