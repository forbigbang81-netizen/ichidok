#!/usr/bin/env python3
"""Download JJK S1 (24 eps) English subtitles from OpenSubtitles — STRICT S01EXX matching."""
...
# Find S1 episodes — STRICT: must match S01EXX (case-insensitive)
# Reject anything that matches S02, S03 etc.
eps = {}
for r in unique:
    ...
    if "jujutsu" not in mname and "jujutsu" not in fname.lower():
        continue
    # MUST match S01EXX — reject S02/S03 etc
    m = re.search(r'[Ss]01[Ee](\d+)', fname)
    if not m:
        continue
    # Reject if filename also contains S02 or S03 (some files have both)
    if re.search(r'[Ss]0[23][Ee]\d+', fname):
        continue
    ep = int(m.group(1))
    if ep < 1 or ep > 24:
        continue
    # Score: prefer SRT, prefer Crunchyroll/Netflix source
    score = 0
    fl = fname.lower()
    if fl.endswith(".srt"):
        score += 20
...
for ep in missing:
    ...
    # Try search by episode number + anime name
    for q in [f"jujutsu kaisen s01e{ep:02d}", f"jujutsu kaisen {ep}"]:
        url = f"https://rest.opensubtitles.org/search/query-{urllib.request.quote(q)}/sublanguageid-eng"
        try:
            results = fetch_json(url)
            print(f"    Query '{q}': {len(results)} results")
            for r in results:
                fname = r.get("SubFileName", "")
                # Must match S01EXX or just be E{ep}
                m = re.search(r'[Ss]01[Ee](\d+)', fname)
                if not m:
                    # Try plain E{ep:02d} at word boundary
                    m2 = re.search(rf'[Ee]0?{ep}\b', fname)
                    if not m2:
                        continue
                    # But reject if S02 or S03 in name
                    if re.search(r'[Ss]0[23][Ee]\d+', fname):
                        continue
                else:
                    if int(m.group(1)) != ep:
