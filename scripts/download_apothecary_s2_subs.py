#!/usr/bin/env python3
"""Download Apothecary Diaries S2 (24 eps) English subtitles from OpenSubtitles."""
...
def is_s2_episode(fname, target_ep):
    """Check if filename is Apothecary Diaries S2 episode `target_ep`."""
    fl = fname.lower()
    # Reject S01, S1, Season 1
    if re.search(r'[Ss]0?1[Ee]\d+', fl):
        return False
    if re.search(r'season\s*1\b', fl):
        return False
    # Reject S03+
    if re.search(r'[Ss]0[3456789]', fl):
        return False
    if re.search(r'season\s*[3456789]', fl):
        return False
    # Accept S02E{ep}
    m = re.search(r'[Ss]02[Ee](\d+)', fl)
    if m:
        return int(m.group(1)) == target_ep
    # Accept S2E{ep}
    m = re.search(r'[Ss]2[Ee](\d+)', fl)
    if m:
        return int(m.group(1)) == target_ep
    return False

# Use broad search to find all Apothecary Diaries subtitles
queries = ["apothecary diaries", "kusuriya no hitorigoto", "kusuriya"]
all_results = []
for q in queries:
...
for r in unique:
    ...
    if "apothecary" not in mname and "apothecary" not in fname.lower() and "kusuriya" not in mname and "kusuriya" not in fname.lower():
        continue
    for ep in range(1, 25):
        if is_s2_episode(fname, ep):
            score = 0
            fl = fname.lower()
            if fl.endswith(".srt"):
                score += 20
...
print(f"\nS2 episodes matched (broad search): {sorted(eps.keys())}")
...
for ep in missing:
    ...
    for q in [f"apothecary diaries s02e{ep:02d}", f"kusuriya no hitorigoto s02e{ep:02d}", f"apothecary diaries {ep}"]:
        url = f"https://rest.opensubtitles.org/search/query-{urllib.request.quote(q)}/sublanguageid-eng"
        try:
            results = fetch_json(url)
            for r in results:
...
print(f"\nFinal S2 episodes matched: {sorted(eps.keys())}")
...
print(f"\n>>> Apothecary Diaries S2: {success}/24 episodes downloaded")
